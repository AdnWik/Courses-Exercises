...
with open('Python+SQL(sqlite3)\S4_PROJEKT_Platforma_e-lerningowa\S5_One-To-Many\S5L13.sql', 'r', encoding='utf-8') as file:
    sql = file.read()
...
cur.execute("""
    SELECT t1.instructor_id,
        t2.first_name,
        t2.last_name,
        COUNT(instructor_id) AS num_courses
    FROM esmartdata_course AS t1 LEFT JOIN esmartdata_instructor AS t2 ON t1.instructor_id == t2.id
